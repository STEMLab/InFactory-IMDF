...
import edu.pnu.stem.dao.KioskSpaceDAO;
...
@RestController
@RequestMapping("/documents/{docId}/KioskSpace")
public class KioskSpaceController {
	...
	public void createKioskSpace(@PathVariable("id") String id, @RequestBody ObjectNode json,
		...
		try {
			...
			c = KioskSpaceDAO.createKioskSpace(map, parentId, id, name, description, geofenceGeometry, duality, level,
					partialBoundedBy, featureInformation, anchor_id, level_id, feature_name, alt_name);
	...
	public void getKioskSpace(@PathVariable("docId") String docId, @PathVariable("id") String id,
			HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		try {
			...
			ObjectNode target = Convert2Json.convert2JSON(map, KioskSpaceDAO.readKioskSpace(map, id));
	...
	public void updateKioskSpace(@PathVariable("docId") String docId, @PathVariable("id") String id,
		...
		try {
			...
			KioskSpaceDAO.updateKioskSpace(map, parentId, id, null, null, geom, duality, partialBoundedBy);
	...
	public void deleteKioskSpace(@PathVariable("docId") String docId, @PathVariable("id") String id,
			@RequestBody ObjectNode json, HttpServletRequest request, HttpServletResponse response) {
		try {
			Container container = applicationContext.getBean(Container.class);
			IndoorGMLMap map = container.getDocument(docId);
			KioskSpaceDAO.deleteKioskSpace(map, id);
		} catch (NullPointerException e) {
			e.printStackTrace();
			throw new UndefinedDocumentException();
		}
