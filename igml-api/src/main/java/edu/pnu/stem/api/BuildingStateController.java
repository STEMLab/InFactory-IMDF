...
import edu.pnu.stem.dao.BuildingStateDAO;
import edu.pnu.stem.dao.BuildingStateDAO;
...
@RestController
@RequestMapping("/documents/{docId}/BuildingState")
public class BuildingStateController {
	...
	public void createBuildingState(@PathVariable("docId") String docId, @PathVariable("id") String id,
		...
		if (json.has("imdf")) {
			...
			if (json.get("imdf").has("category")) {
				...
				for (BUILDINGCATEGORY value : BUILDINGCATEGORY.values()) {
		...
		try {
			...
			s =BuildingStateDAO.createBuildingState(map, parentId, id, name, description, geometry, duality, connected, category, restriction, address_id);
	...
	public void updateBuildingState(@PathVariable("docId") String docId, @PathVariable("id") String id,
			@RequestBody ObjectNode json, HttpServletRequest request, HttpServletResponse response) {
		try {
			Container container = applicationContext.getBean(Container.class);
			IndoorGMLMap map = container.getDocument(docId);
			...
			BuildingStateDAO.updateBuildingState(map, parentId, id, name, description, geom, duality, connects);
	...
	public void getBuildingState(@PathVariable("docId") String docId, @PathVariable("id") String id,
			HttpServletRequest request, HttpServletResponse response) throws IOException {
		try {
			Container container = applicationContext.getBean(Container.class);
			IndoorGMLMap map = container.getDocument(docId);

			ObjectNode target = Convert2Json.convert2JSON(map,BuildingStateDAO.readBuildingState(map, id));
	...
	public void deleteBuildingState(@PathVariable("docId") String docId, @PathVariable("id") String id,
			@RequestBody ObjectNode json, HttpServletRequest request, HttpServletResponse response) {
		try {
			Container container = applicationContext.getBean(Container.class);
			IndoorGMLMap map = container.getDocument(docId);
			BuildingStateDAO.deleteBuildingState(map, id);
		} catch (NullPointerException e) {
			e.printStackTrace();
			throw new UndefinedDocumentException();
		}
