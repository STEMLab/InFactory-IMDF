...
import edu.pnu.stem.dao.GeofenceSpaceDAO;
...
@RestController
@RequestMapping("/documents/{docId}/GeofenceSpace")
public class GeofenceSpaceController {
	...
	public void createGeofenceSpace(@PathVariable("id") String id, @RequestBody ObjectNode json,
		...
		if (json.has("imdf")) {
			...
			if (json.get("imdf").has("category")) {
				...
				for (GEOFENCECATEGORY value : GEOFENCECATEGORY.values()) {
					if (str.toUpperCase().equals(value.toString())) {
						category = value;
			...
			if (json.get("imdf").has("level_ids")) {

				JsonNode level_id = json.get("imdf").get("level_ids");
				level_ids = new String[level_id.size()];

				for (int i = 0; i < level_id.size(); i++) {
					level_ids[i] = level_id.get(i).asText().trim();

				}
			}
			if (json.get("imdf").has("parents")) {

				JsonNode parent = json.get("imdf").get("parents");
				for (int i = 0; i < parent.size(); i++) {
					parents = new String[parent.size()];
					parents[i] = parent.get(i).asText().trim();
		...
		GeofenceSpace c = null;
		try {
			Container container = applicationContext.getBean(Container.class);
			IndoorGMLMap map = container.getDocument(docId);

			c = GeofenceSpaceDAO.createGeofenceSpace(map, parentId, id, name, description, geofenceGeometry, duality, level,
					partialBoundedBy, featureInformation, category, restriction, correlation_id, building_ids,
					level_ids, parents, feature_name, alt_name);
	...
	public void getGeofenceSpace(@PathVariable("docId") String docId, @PathVariable("id") String id,
			HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		try {
			...
			ObjectNode target = Convert2Json.convert2JSON(map, GeofenceSpaceDAO.readGeofenceSpace(map, id));
	...
	public void updateGeofenceSpace(@PathVariable("docId") String docId, @PathVariable("id") String id,
		...
		try {
			...
			GeofenceSpaceDAO.updateGeofenceSpace(map, parentId, id, null, null, geom, duality, partialBoundedBy);
	...
	public void deleteGeofenceSpace(@PathVariable("docId") String docId, @PathVariable("id") String id,
			@RequestBody ObjectNode json, HttpServletRequest request, HttpServletResponse response) {
		try {
			Container container = applicationContext.getBean(Container.class);
			IndoorGMLMap map = container.getDocument(docId);
			GeofenceSpaceDAO.deleteGeofenceSpace(map, id);
		} catch (NullPointerException e) {
			e.printStackTrace();
			throw new UndefinedDocumentException();
		}
