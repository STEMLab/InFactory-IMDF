...
import edu.pnu.stem.dao.SectionSpaceDAO;
...
@RestController
@RequestMapping("/documents/{docId}/SectionSpace")
public class SectionSpaceController {
	...
	public void createSectionSpace(@PathVariable("id") String id, @RequestBody ObjectNode json,
		...
		if (json.has("imdf")) {
			...
			if (json.get("imdf").has("category")) {
				...
				for (SECTIONCATEGORY value : SECTIONCATEGORY.values()) {
					if (str.toUpperCase().equals(value.toString())) {
						category = value;
			...
			if (json.get("imdf").has("address_id")) {
				...
				level_id = json.get("imdf").get("address_id").asText().trim();
		...
		try {
			...
			c = SectionSpaceDAO.createSectionSpace(map, parentId, id, name, description, geofenceGeometry, duality,
					level, partialBoundedBy, featureInformation, category, restriction, accessibility, level_id,
					address_id, feature_name, alt_name, parents);
	...
	public void getSectionSpace(@PathVariable("docId") String docId, @PathVariable("id") String id,
			HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		try {
			...
			ObjectNode target = Convert2Json.convert2JSON(map, SectionSpaceDAO.readSectionSpace(map, id));
	...
	public void updateSectionSpace(@PathVariable("docId") String docId, @PathVariable("id") String id,
		...
		try {
			...
			SectionSpaceDAO.updateSectionSpace(map, parentId, id, null, null, geom, duality, partialBoundedBy);
	...
	public void deleteSectionSpace(@PathVariable("docId") String docId, @PathVariable("id") String id,
			@RequestBody ObjectNode json, HttpServletRequest request, HttpServletResponse response) {
		try {
			Container container = applicationContext.getBean(Container.class);
			IndoorGMLMap map = container.getDocument(docId);
			SectionSpaceDAO.deleteSectionSpace(map, id);
		} catch (NullPointerException e) {
			e.printStackTrace();
			throw new UndefinedDocumentException();
		}
