...
public class Convert2FeatureClass {
	...
	public static IndoorFeaturesForIMDF change2FeatureClass(IndoorGMLMap savedMap, String docId,
			IndoorFeaturesForIMDFType feature) throws JAXBException {
		// Creating this feature
		IndoorFeaturesForIMDF newFeature = (IndoorFeaturesForIMDF) savedMap.getFeature(feature.getId());
		if (newFeature == null) {
			newFeature = new IndoorFeaturesForIMDF(savedMap, feature.getId());
			savedMap.setFeature(feature.getId(), "IndoorFeaturesForIMDF", newFeature);
		}
		// Creating containing features
		PrimalSpaceFeatures childP = change2FeatureClass(savedMap,
				feature.getPrimalSpaceFeatures().getPrimalSpaceFeatures(), feature.getId());
	...
	public static IndoorFeatures change2FeatureClass(IndoorGMLMap savedMap, String docId, IndoorFeaturesType feature)
	...
	public static Object change2FeatureClass(IndoorGMLMap savedMap, String parentId,
			CellSpaceBoundaryGeometryType feature) {
		Object newFeature = null;
		if (feature.isSetGeometry2D()) {
	...
	public static CellSpace change2FeatureClass(IndoorGMLMap savedMap, CellSpaceType feature, String parentId) {
		...
		if (cellSpaceGeom != null) {
			...
			if (cellSpaceGeom.isSetGeometry2D()) {
				Polygon geom = Convert2JTSGeometry.convert2Polygon(
						(PolygonType) feature.getCellSpaceGeometry().getGeometry2D().getAbstractSurface().getValue());
				// GeometryUtil.setMetadata(geom, "id",
				// feature.getCellSpaceGeometry().getGeometry2D().getAbstractSurface().getValue().getId());
				newFeature.setGeometry(geom);
			} else if (cellSpaceGeom.isSetGeometry3D()) {
				Solid geom = Convert2JTSGeometry.Convert2Solid(
						(SolidType) feature.getCellSpaceGeometry().getGeometry3D().getAbstractSolid().getValue());
				// GeometryUtil.setMetadata(geom, "id",
				// feature.getCellSpaceGeometry().getGeometry3D().getAbstractSolid().getValue().getId());
				newFeature.setGeometry(geom);
			}
	...
	public static CellSpaceBoundary change2FeatureClass(IndoorGMLMap savedMap, CellSpaceBoundaryType feature,
			String parentId) {
		// Creating this feature
	...
	public static MultiLayeredGraph change2FeatureClass(IndoorGMLMap savedMap, MultiLayeredGraphType feature,
			String parentId) {
		// Creating this feature
	...
	public static Edges change2FeatureClass(IndoorGMLMap savedMap, EdgesType feature, String parentId) {
		...
		for (TransitionMemberType tmType : tms) {
			TransitionType tType = tmType.getTransition().getValue();
	...
	public static InterLayerConnection change2FeatureClass(IndoorGMLMap savedMap, InterLayerConnectionType feature,
			String parentId) {
		// Creating this feature
	...
	public static PrimalSpaceFeatures change2FeatureClass(IndoorGMLMap savedMap, PrimalSpaceFeaturesType feature,
			String parentId) throws JAXBException {
		// Creating this feature
	...
	public static Transition change2FeatureClass(IndoorGMLMap savedMap, TransitionType feature, String parentId) {
		...
		// 2. geometry
		if (feature.isSetGeometry()) {
			LineString geom = Convert2JTSGeometry
					.convert2LineString((LineStringType) feature.getGeometry().getAbstractCurve().getValue());
	...
	public static AnchorBoundary change2FeatureClass(IndoorGMLMap savedMap, AnchorBoundaryType feature,
			String parentId) {
		// Creating this feature
