...
public class BuildingStateDAO {
	public static BuildingState readBuildingState(IndoorGMLMap map, String id) {
		BuildingState target = null;
		try {
			target = (BuildingState) map.getFeature(id);
	...
	public static BuildingState createBuildingState(IndoorGMLMap map, String parentId, String id, String name,
			String description, Geometry geometry, String duality, List<String> connected, BUILDINGCATEGORY category,
			RESTRICTIONCATEGORY restriction, String address_id) {

		if (id == null) {
			id = UUID.randomUUID().toString();
		}

		BuildingState newFeature = new BuildingState(map, id);
		if (map.hasFutureID(id)) {
			newFeature = (BuildingState) map.getFutureFeature(id);
		...
		if (category != null) {
			newFeature.setBuildingCategory(category);
		}
		if (restriction != null) {
	...
	public static State createBuildingState(IndoorGMLMap map, String parentId, String id, String geometry,
	...
	public static void deleteBuildingState(IndoorGMLMap map, String id) {
		State target = (State) map.getFeature(id);

		if (target == null) {
			if (map.hasFutureID(id))
				target = (BuildingState) map.getFutureFeature(id);
		}
	...
	public static State updateBuildingState(IndoorGMLMap map, String parentId, String id, String name,
			String description, Geometry geometry, String duality, List<String> connects) {
		...
		map.removeFeature(id);
		map.setFeature(id, "BuildingState", result);
