...
public class SectionSpaceDAO {
	public static void deleteSectionSpace(IndoorGMLMap map, String id) {

		SectionSpace target = (SectionSpace) map.getFeature(id);
	...
	public static SectionSpace readSectionSpace(IndoorGMLMap map, String id) {

		SectionSpace target = null;
		try {
			target = (SectionSpace) map.getFeature(id);
	...
	public static SectionSpace updateSectionSpace(IndoorGMLMap map, String parentId, String id, String name,
			String description, Geometry geometry, String duality, List<String> partialboundedBy) {
		SectionSpace result = new SectionSpace(map, id);
		SectionSpace target = (SectionSpace) map.getFeature(id);
		...
		map.removeFeature(id);
		map.setFeature(id, "SectionSpace", result);

		return result;
	}

	public static SectionSpace createSectionSpace(IndoorGMLMap map, String parentId, String id, String name,
			String description, Geometry geometry, String duality, List<String> level, List<String> partialBoundedBy,
			FeatureInformation featureInformation, SECTIONCATEGORY category, RESTRICTIONCATEGORY restriction, ACCESSIBILITYCATEGORY[] accessibility,  String level_id, String address_id, Labels feature_name,
			Labels alt_name, String[] parents) {

		if (id == null) {
			id = UUID.randomUUID().toString();
		}

		SectionSpace newFeature = new SectionSpace(map, id);

		if (map.hasFutureID(id)) {
			newFeature = (SectionSpace) map.getFutureFeature(id);
			// map.removeFutureID(id);
		} else {
			map.setFutureFeature(id, newFeature);
		}
		map.setFeature(id, "SectionSpace", newFeature);
