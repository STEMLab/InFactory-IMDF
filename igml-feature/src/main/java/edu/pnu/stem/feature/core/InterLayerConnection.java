...
public class InterLayerConnection extends AbstractFeature {
	...
	public SpaceLayer[] getConnectedLayers() {
		...
		if((SpaceLayer)indoorGMLMap.getFeature(this.connectedLayers[0])!=null) {
			found[0] = (SpaceLayer)indoorGMLMap.getFeature(this.connectedLayers[0]);
		}
		else {
			System.out.println("1st ConnectedLayers of InterLayerConnection ID is wrong");
		}
		if((SpaceLayer)indoorGMLMap.getFeature(this.connectedLayers[1])!=null) {
			found[1] = (SpaceLayer)indoorGMLMap.getFeature(this.connectedLayers[1]);
		}
		else {
			System.out.println("2nd ConnectedLayers of InterLayerConnection ID is wrong");
		}	
	...
	public State[] getInterConnects() {
		...
		if(this.interConnects != null) {
			found = new State[2];
			if(indoorGMLMap.getFeature(this.interConnects[0])!=null) {
				found[0] = (State)indoorGMLMap.getFeature(this.interConnects[0]);
			}
			else {
				System.out.println("1st InterConnects of InterLayerConnection ID is wrong");
			}
			if(indoorGMLMap.getFeature(this.interConnects[1])!=null) {
				found[1] = (State)indoorGMLMap.getFeature(this.interConnects[1]);
			}
			else {
				System.out.println("2nd InterConnects of InterLayerConnection ID is wrong");
			}
